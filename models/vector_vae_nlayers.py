...
class VectorVAEnLayers(VectorVAE):
    ...
    def __init__(self,
        ...
        self.colors = [[0, 0, 0, 1], [255/255, 165/255, 0/255, 1], [255/255, 165/255, 0/255, 1],]
        self.divide_shape = nn.Sequential(
            get_computational_unit(latent_dim+3, latent_dim, 'mlp'),
    ...
    def forward(self, input: Tensor, **kwargs) -> List[Tensor]:
        ...
        output, overlap_loss = self.decode_and_composite(z, verbose=False, return_overlap_loss=True)
        return [output, input, mu, log_var, overlap_loss]
    ...
    def decode_and_composite(self, z: Tensor, return_overlap_loss=False, **kwargs):
        ...
        for i in range(3):
            ...
            layers.append(layer)
        # sum_alpha = layers[0][:,3:4,:,:] + layers[1][:,3:4,:,:]*1000 + layers[2][:,3:4,:,:]*1000 + 0.0001
        # alpha_0 = layers[0][:,3:4,:,:]/ sum_alpha
        # alpha_1 = layers[1][:,3:4,:,:]*1000/ sum_alpha
        # alpha_2 = layers[2][:,3:4,:,:]*1000/ sum_alpha
        # output = (layers[0][:, :3]*alpha_0) + \
        #          (layers[1][:, :3]*alpha_1) +\
        #          (layers[2][:, :3]*alpha_2) +\
        #          ((1-alpha_0)*(1-alpha_1)*(1-alpha_2))
        output = (layers[0][:, :3]*layers[0][:,3:4,:,:]*(1-layers[1][:,3:4,:,:])*(1-layers[2][:,3:4,:,:])) + \
