...
class VectorVAEnLayers(VectorVAE):
    ...
    def __init__(self,
        ...
        self.colors = [[0, 0, 0, 1], [0/255, 255/255, 255/255, 1], [255/255, 0/255, 255/255, 1],]
    ...
    def hard_composite(self, **kwargs):
        ...
        # rgb = layers[n - 1][:, :3] * alpha
        # for i in range(n - 2, -1, -1):
        #     rgb = rgb + layers[n - 1][:, :3] * layers[i][:, 3:4, :, :] * (1 - alpha)
        #     alpha = alpha + layers[i][:, 3:4, :, :] * (1 - alpha)
        # rgb = (layers[0][:, :3] * layers[0][:, 3:4, :, :]*(1 - layers[3][:, 3:4, :, :]) * (1 - layers[1][:, 3:4, :, :]) * (
        #             1 - layers[2][:, 3:4, :, :])) + \
        #          (layers[1][:, :3] * layers[1][:, 3:4, :, :]*(1 - layers[3][:, 3:4, :, :]) * (1 - layers[2][:, 3:4, :, :])) + \
        #          (layers[2][:, :3]*(1 - layers[3][:, 3:4, :, :]) * layers[2][:, 3:4, :, :]) + \
        #          (layers[3][:, :3] * layers[3][:, 3:4]) + \
        #          ((1 - layers[0][:, 3:4, :, :]) * (1 - layers[1][:, 3:4, :, :]) * (1 - layers[2][:, 3:4, :, :]) * (1 - layers[3][:, 3:4]))
        rgb = (layers[0][:, :3] * layers[0][:, 3:4, :, :] * (1 - layers[1][:, 3:4, :, :]) * (
                    1 - layers[2][:, 3:4, :, :])) + \
    ...
    def decode_and_composite(self, z: Tensor, return_overlap_loss=False, **kwargs):
        ...
        # inv_mask = ((1 - layers[0][:, 3:4, :, :]) * (1 - layers[1][:, 3:4, :, :]) * (1 - layers[2][:, 3:4, :, :]))
        # sum_alpha = layers[0][:, 3:4, :, :]*z_layers[0] + layers[1][:, 3:4, :, :]*z_layers[1] + layers[2][:, 3:4, :, :]*z_layers[2] + inv_mask
        # alpha0 = layers[0][:, 3:4, :, :]*z_layers[0] / sum_alpha
        # alpha1 = layers[1][:, 3:4, :, :]*z_layers[1] / sum_alpha
        # alpha2 = layers[2][:, 3:4, :, :]*z_layers[2] / sum_alpha
        # inv_mask = inv_mask/sum_alpha
